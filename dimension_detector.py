...
Detecta las dimensiones de archivos PDF e imágenes en varias unidades
(píxeles, pulgadas, centímetros).
...
------------
    ...
    DimensionDetector (ABC)           -> interfaz común (patrón Strategy)
        ImageDimensionDetector        -> implementación para imágenes (Pillow)
        PDFDimensionDetector          -> implementación para PDF (pypdf)
    DetectorFactory                   -> elige el detector correcto (patrón Factory)
    DimensionAnalyzer                 -> fachada de alto nivel (Facade)
    CLI (main)                        -> interfaz de línea de comandos

Para agregar soporte a un nuevo formato: crear una clase que herede de
DimensionDetector, implementar supports() y detect(), y registrarla en
DetectorFactory._detectors. Nada más del código necesita tocarse.

NOTA IMPORTANTE SOBRE "100% DE PRECISIÓN"
------------------------------------------
- En una IMAGEN, el tamaño en píxeles es un dato exacto (viene en el
  header del archivo). El tamaño en pulgadas/cm depende del DPI
  guardado en los metadatos (si existe); si no existe, se asume un DPI
  por defecto y el resultado se marca como "estimado".
- Un PDF es un formato VECTORIAL: no contiene píxeles. El tamaño de
  página en puntos (1 pt = 1/72 in) SÍ es exacto (viene del MediaBox),
  y por lo tanto in/cm derivados de ahí también lo son. El valor en
  píxeles solo tiene sentido si defines a qué DPI se rasterizaría la
  página, así que siempre se marca como estimado.
...
---
    ...
    python dimension_detector.py *.png --json
"""
